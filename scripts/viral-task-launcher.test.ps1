...
foreach($needle in @('LocalApplicationData','viral-task-launcher.log','code=','runner_start','runner_exit_','launcher_error_','System32\cmd.exe','run-viral-loop.cmd','call "','PRISMBAY_LOG_DIR','CJ_MCP_ENABLE','TOKEN_ENCRYPT_KEY','Remove-Item Env:TOKEN_ENCRYPT_KEY')){
  if(-not $l.Contains($needle)){throw ('Missing launcher safeguard: '+$needle)}
}
foreach($needle in @('PRISMBAY_LOG_DIR','LOCALAPPDATA','PrismBay','viral-refresh.log','mkdir "%LOGDIR%"','git pull --rebase origin main','render-viral-queue.mjs')){
